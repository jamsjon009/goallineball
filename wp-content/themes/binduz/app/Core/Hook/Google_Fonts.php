...
class Google_Fonts {
	...
	public static function generate_url() {
		...
		foreach ( self::$data[ 'family' ] as $family => $family_data ) {
			if ( !empty( $family ) ) {
				$family_data[ 'variants' ][900] = $family_data[ 'variants' ][700] = $family_data[ 'variants' ][400] = $family_data[ 'variants' ][500] = 1;
	...
	public static function output_url() {
		if ( $url = self::generate_url() ):
		  wp_enqueue_style( 'binduz-google-fonts', esc_attr( $url ), null, BINDUZ_VERSION );
		endif;
	}
