...
function wpforo_activity_content( $item = [] ) {
	...
	$args['user_id'] = ( isset( $item['userid'] ) && $item['userid'] ) ? $item['userid'] : $args['user_id'] = WPF(
	)->current_userid;
...
function wpforo_share_toggle( $url = '', $text = '', $location = 'side', $custom = false ) {
	$position = in_array( wpforo_setting( 'social', 'sb_location_toggle' ), [ 'left', 'right' ]
	) ? 'side' : wpforo_setting( 'social', 'sb_location_toggle' );
					...
					<?php foreach( $tags as $tag ): ?>
                        ...
                        <tag wpf-tooltip="<?php echo esc_attr( wpforo_phrase( 'Topic Tag', false ) ); ?>"><a
                                    href="<?php echo wpforo_home_url(
									                 ) . '?wpfin=tag&wpfs=' . $tag ?>"><?php echo esc_html(
									$tag
								); ?><?php if( $count && wpfval(
										$item,
										'count'
	...
	if( ! empty( $topic ) ) {
		if( wpfval( $topic, 'topicid' ) && wpfval( $topic, 'forumid' ) ) {
			...
			$navi_topics = WPF()->db->get_col(
				"SELECT `topicid` FROM `" . WPF(
				)->tables->topics . "` WHERE ( `topicid` = IFNULL((SELECT min(`topicid`) FROM `" . WPF(
				)->tables->topics . "` WHERE `topicid` > " . intval( $topic['topicid'] ) . " AND `forumid` = " . intval(
					$topic['forumid']
				) . " AND `status` = 0 AND `private` = 0),0) OR `topicid` = IFNULL((SELECT max(`topicid`) FROM `" . WPF(
				)->tables->topics . "` WHERE `topicid` < " . intval( $topic['topicid'] ) . " AND `forumid` = " . intval(
					$topic['forumid']
			...
			if( ! empty( $forums ) ) {
				foreach( $forums as $forum ) $forum_urls[ 'forum_' . $forum['forumid'] ] = wpforo_home_url(
					$forum['slug']
				);
			}
			?>
...
function wpforo_topic_footer() {
	if( wpfval( WPF()->current_object, 'topic' ) && wpfval( WPF()->current_object, 'template' ) && WPF(
...
function wpforo_is_member_template( $template = null ) {
	if( $template = WPF()->tpl->get_template( $template ) ) return wpfkey(
...
function wpforo_get_not_replied_topicids() {
	$sql = "SELECT `topicid`, COUNT(DISTINCT userid) AS userid_count FROM `" . WPF(
		)->tables->posts . "` GROUP BY `topicid` HAVING userid_count = 1";
...
function wpforo_profile_head_attrs( $user ) {
	printf(
		...
		esc_url(
			trim( (string) wpfval( $user, 'cover' ) ) ? trim( (string) wpfval( $user, 'cover' ) ) . '?lm=' . time(
				) : wpforo_setting( 'profiles', 'default_cover' )
		)
	);
}
...
function wpforo_get_forum_cover_styles( $forum ) {
	...
	if( $forum['cover_url'] ) {
		$border_radius = ( (int) $forum['layout'] !== 4 ) ? 'border-radius: 4px;' : ( ! is_rtl(
		...
		$r['button']   = 'style="padding-top:3px; padding-bottom: 2px; line-height: 26px; ' . ( ! is_rtl(
