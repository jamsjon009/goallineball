...
class Forms {
	...
	public function generate_multi_item( $val, $dvalues = [], $save_button = false ) {
		return sprintf(
			...
			$val,
			( $save_button ? sprintf( '<span class="wpf-multi-item-action wpf-save-variant" title="%1$s"><i class="fa-regular fa-floppy-disk" title="%1$s"></i></span>', wpforo_phrase( 'Add to pre-defined values', false ) ) : '' ),
	...
	public function field_avatar( $f ) {
		...
		$field_html = '<ul>
				...
				<li>
				    <input name="' . esc_attr( $f['varname'] ) . '[avatar_type]" id="wpfat_remote" value="remote" ' . ( $f['value'] && strpos( (string) $f['value'], 'wpforo/avatars' ) === false ? 'checked="checked"' : '' ) . ' type="radio" />&nbsp;
