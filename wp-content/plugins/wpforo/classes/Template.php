...
class Template {
	...
	private function fix_template( $template ) {
		if( is_string( $template ) ) {
			...
			if( is_array( $template ) ) {
				...
				if( ! $template['ico'] = trim( (string) $template['ico'] ) ) $template['ico'] = '<i class="fas fa-user"></i>';
        ...
        <div class="wpf-breadcrumb" itemscope="" itemtype="https://schema.org/BreadcrumbList">
			<?php switch( $current_object['template'] ) :
				...
				case 'account': ?>
					...
					<?php if( $current_object['user'] ) : ?>
                        <div itemprop="itemListElement" itemscope="" itemtype="https://schema.org/ListItem" class="wpf-item-element"><a itemprop="item" href="<?php echo esc_url( (string) $current_object['user']['profile_url'] ) ?>"><span itemprop="name"><?php wpforo_text( wpforo_user_dname( $current_object['user'] ), $lenght ) ?></span></a>
                        ...
                        foreach( $relative_ids as $key => $rel_forumid ) : ?>
                            ...
                            <?php if( ! empty( $forum ) ): ?>
                                <?php if( $key != ( count( $relative_ids ) - 1 ) ) : ?>
                                    <div itemprop="itemListElement" itemscope="" itemtype="https://schema.org/ListItem" class="wpf-item-element"><a itemprop="item" href="<?php echo esc_url( (string) $forum['url'] ) ?>" title="<?php echo esc_attr( $forum['title'] ) ?>"><span itemprop="name"><?php wpforo_text( $forum['title'], $lenght ) ?></span></a>
				...
				case 'post': ?>
					...
					<?php if( $current_object['forumid'] ) : ?>
						...
						foreach( $relative_ids as $key => $rel_forumid ) : ?>
							<?php $forum = wpforo_forum( $rel_forumid ) ?>
							<?php if( ! empty( $forum ) ): ?>
                                <div class="wpf-item-element" itemprop="itemListElement" itemscope="" itemtype="https://schema.org/ListItem"><a itemprop="item" href="<?php echo esc_url( (string) $forum['url'] ) ?>" title="<?php echo esc_attr( $forum['title'] ) ?>"><span itemprop="name"><?php wpforo_text( $forum['title'], $lenght ) ?></span></a>
                                    <meta itemprop="position" content="<?php echo $key + 2; ?>">
                                </div>
							<?php endif ?>
						<?php endforeach ?>
	...
	public function member_social_buttons( $member ) {
		...
		if( $social_access ) {
			...
			if( isset( $member['facebook'] ) && $member['facebook'] ) {
				...
				$member['facebook']              = ( strpos( (string) $member['facebook'], 'facebook.com' ) === false ) ? 'https://www.facebook.com/' . trim( (string) $member['facebook'], '/' ) : $member['facebook'];
			...
			if( isset( $member['twitter'] ) && $member['twitter'] ) {
				...
				$member['twitter']              = ( strpos( (string) $member['twitter'], 'twitter.com' ) === false ) ? 'https://twitter.com/' . trim( (string) $member['twitter'], '/' ) : $member['twitter'];
            ...
            <div class="wpf-member-socnet-wrap">
				<?php if( ! empty( $socnets ) ): ?>
					<?php foreach( $socnets as $key => $socnet ): ?>
						...
						<?php $url = ( $key == 'skype' ) ? 'skype:' . esc_attr( $socnet['value'] ) : esc_url( (string) $socnet['protocol'] . str_replace( [ 'https://', 'http://', 'skype:', 'mailto:' ], '', $socnet['value'] ) ); ?>
                        <a href="<?php echo $url ?>" class="wpf-member-socnet-button" title="<?php echo esc_attr( $title ) ?>">
                            <img src="<?php echo esc_url( (string) WPFORO_URL ) ?>/assets/images/sn/<?php echo $key ?>.png" alt="<?php echo esc_attr( $title ) ?>" title="<?php echo esc_attr( $title ) ?>"/>
                        </a>
					<?php endforeach; ?>
				<?php endif; ?>
	...
	function show_layout_selectbox( $layoutid = 0 ) {
		...
		if( ! empty( $layouts ) ) {
			foreach( $layouts as $layout ) : ?>
                <option value="<?php echo esc_attr( trim( (string) $layout['id'] ) ) ?>" <?php echo( $layoutid == $layout['id'] ? 'selected' : '' ); ?> ><?php echo esc_html( $layout['name'] ) ?></option>
			<?php
			endforeach;
		}
	}
